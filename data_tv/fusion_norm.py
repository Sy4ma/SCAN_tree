...
def normaliseSimiarities(sims):
    sims_mins = np.min(sims, 1)
    sims_maxs = np.max(sims, 1)
    sims_mins = sims_mins[:,np.newaxis]
    sims_maxs = sims_maxs[:,np.newaxis]
    return (sims - sims_mins) / (sims_maxs - sims_mins)


if len(sys.argv) != 6:
    print(
        "Usage: python fusion_norm.py <Result paths separeted by comma>"
...
for resultDir in allResultDirs:
    ...
    simsFinal += normaliseSimiarities(sims)
    # set_trace()
...
if outputFusionResult != 0:
    ...
    simsFinal_norm = normaliseSimiarities(simsFinal)
    np.save(outputFusionResultFilename, simsFinal_norm)
    # set_trace()
